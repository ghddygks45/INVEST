...
function fn_action_printing() {
	window.open('print.do', '', 'width=1280,height=900,left=0,scrollbars=yes');
	return false;
}
...
function fn_download_pdf() {
	window.open('download-pdf.do', '', 'width=1280,height=900,left=0,scrollbars=yes');
	return false;
}
...
function fn_download_img() {
	window.open('download-img.do', '', 'width=1280,height=900,left=0,scrollbars=yes');
	return false;
}
...
$(document).ready(function () {
	...
	var share_sns = function (_this) {
		...
		if (sns_type == 'facebook') {
			...
			loc = 'https://www.linkedin.com/shareArticle?mini=true&url=' + href + '&title=' + title;
		}
...
/*--- [s] 20220502 gnb 영역 수정 ---*/
$(function () {
	if ($('html').attr('class') == 'desktop') {
		$(document).on('mouseenter focusin', '.desktop .gnb > ul > li > a', function () {
			var target1 = $(this).siblings().find(".gnb-box>ul>li>ul");
			// .over_depth 넣기
			target1.each(function () {
				if ($(this).children("li").length > 9) {
					$(this).parent().addClass("over_depth");
				}
			})

			// depth 나누기
			var target2 = $(this).siblings().find(".gnb-box>ul>.over_depth>ul>li");
			var target3 = $(this).siblings().find(".gnb-box>ul>.over_depth");

			target3.each(function () {
				for (var i = 0; i < target2.length; i += 9) {
					$("<ul></ul>").append(target2.slice(i + 9)).appendTo(target3);
				}
				$(this).children('ul').wrapAll("<div class='over_depth_wrap'></div>");
				$(".over_depth_wrap>ul:empty").remove();
			})

			// depth 넓이 변경
			var odw = target3.find(">.over_depth_wrap").find(">ul").length;
			if (odw < 3) {
				target3.find(">a").width("470px");
			} else if (odw < 4) {
				target3.find(">a").width("720px");
			}
		})
	}
})
...
$(window).on("resizeEnd", function () {
	...
	if ($(this).width() < 1218) {
		$("html").removeClass("desktop").addClass("mobile");
		$('.over_depth_wrap>ul').unwrap();
		$('.over_depth li').unwrap();
		var target1 = $(".gnb-box>ul").find(">li");
		var target2;
		var returndepth = target1.each(function () {
			if (target1.hasClass("over_depth")) {
