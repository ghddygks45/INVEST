 ...
 // IKMP 매물추천 슬라이더
                  ...
                  function searchBodyGrfiOpen(){
                     ...
                     var makeSearchResult = function(data){
                    	 ...
                    	 $.each(data.searchList,function(i,v){
                    		 if(v.Id == 'news'){
                    			 ...
                    			 $("#news-bbs-cnt").html('A total of <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
                    				 var newsBbs = "";
                    			 ...
                    			 $("#rpt-bbs-cnt").html('A total of <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
									var rptBbs = "";
	                    			 $.each(v.DocumentSet.Document,function(k,d){
	                    				 var regDt = new Date(d.Date);
	                    				 var year = regDt.getFullYear();
						 ...
						 //$("#mna-bbs-cnt").html('A total of <span class="num">'+numberWithCommas(data.mnaCnt)+' data</span>');
						 var mnaBbs = "";
                     ...
                     var makeSearchGfResult = function(data){
						...
						// $("#grfi-bbs-cnt").html('A total of <span class="num">'+numberWithCommas(data.grfiCnt)+' data</span>');
						 var grfiBbs = "";
                         ...
                         $.each(data.grfiList,function(i,v){
							...
							// grfiBbs += '<div class="badge">BEST<span>'+(i+1)+'</span></div>';
							 ...
							 grfiBbs += '<li><span class="bold">Designated area(km2) : </span>'+numberWithCommas(v.appn_ar)+'</li>';
							 grfiBbs += '<li><span class="bold">Nearby railway station : </span>'+v.eng_rlroad_name+'</li>';
							 grfiBbs += '<li><span class="bold">Nearby airports : </span>'+v.eng_arprt_val+'</li>';
							 grfiBbs += '</ul>';
							 grfiBbs += '<button type="button" class="btn-position gfri-info" data-seq="'+v.indust_hsmp_cd+'">View Information</button>';
							grfiBbs += '</div>';
							 grfiBbs += '</a>';
							 grfiBbs += '</div>';
						 });
                     ...
                     var makeBbsList =function(data){
                    	 $("#popup-total-count").html('A total of <span>'+numberWithCommas(data.totCnt)+'</span>data [<span>'+data.paginationInfo.currentPageNo+'</span>/'+data.paginationInfo.totalPageCount+'page]');
                    	 var listHtml = "";
                    	 $.each(data.resultList.Document,function(i,v){
                    		 ...
                    		 listHtml += '<li><span class="bold">Writer : </span>'+v.Field.USER_NAME+'</li>';
                    		 listHtml += '<li><span class="bold">Creation date : </span>'+year+'.'+month+'.'+day+'</li>';
                    		 listHtml += '<li><span class="bold">Views : </span>'+numberWithCommas(v.Field.RDCNT)+'</li>';
                    		 listHtml += '</ul>';
                    		 listHtml += '</div>';
