...
$(document).ready(function () {
	...
	$(document).on("click", ".btn-pop-pdf", function () {
		...
		// 	$(".chart-area>svg").hide()
		// }
		$(".chart-area").css("height", "400px");
		$(".chart-area > svg").attr("style", "width:100% !important");

		html2canvas($('.layer-popup-box.active .popup')[0], {
...
// 맞춤 정보 검색 서비스 툴팁 레이어 닫기
function CloseDay() {
	if ($('#tooltip_check').is(':checked')) {
		toolTipSetCookie('tooltip-pop', "done", 1);
	}
	$('.top_search_link > a').removeClass();
	$('header .tooltip').hide();
}

// 맞춤 정보 검색 서비스 툴팁 레이어 (오늘 하루 보지 않기 체크된 팝업 숨김 처리)
setTimeout(function () {
	if (toolTipGetCookie('tooltip-pop')) { // '오늘 하루 보지 않기 체크'를 했다면
		$('header .tooltip').hide();
	} else {
		$('header .tooltip').show();
	}
...
// 맞춤 정보 검색 서비스 툴팁 레이어 팝업 쿠키 찾기
function toolTipGetCookie(name) {
...
// 맞춤 정보 검색 서비스 툴팁 레이어 팝업 쿠키 생성
function toolTipSetCookie(name, value, expiredays) {
