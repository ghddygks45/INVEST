 ...
 // IKMP 매물추천 슬라이더
                  ...
                  function searchBodyGrfiOpen(){
                     ...
                     var makeSearchResult = function(data){
                    	 ...
                    	 $.each(data.searchList,function(i,v){
                    		 if(v.Id == 'news'){
                    			 ...
                    			 $("#news-bbs-cnt").html('共 <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
                    				 var newsBbs = "";
 	                    			 $.each(v.DocumentSet.Document,function(k,d){
	                    				 ...
	                    				 newsBbs += '<li><a href="'+d.Field.PATH_MENU_URL+'">'+d.Field.SJ+'</a><span class="date">'+year+'.'+mouth+'.'+day+'</span></li>';
                     				 });
 	                    			if(newsBbs == ""){
 	 	      							 $("#news-bbs-list").append("<li>No Data</li>");
 		 	      						 }else{
 		 	      							 $("#news-bbs-list").append(newsBbs);
 		 	      						 }
                    		 }else if(v.Id == 'indust_rpt'){
                    			 $("#rpt-cnt").text(v.DocumentSet.TotalCount);
                    			 $("#rpt-bbs-cnt").html('共 <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
									var rptBbs = "";
	                    			 $.each(v.DocumentSet.Document,function(k,d){
	                    				 var regDt = new Date(d.Date);
	                    				 var year = regDt.getFullYear();
	                    				 var mouth = ('0'+(1 + regDt.getMonth())).slice(-2);
	                    				 var day = ('0'+regDt.getDate()).slice(-2);
	                    				 rptBbs += '<li><a href="'+d.Field.PATH_MENU_URL+'">'+d.Field.SJ+'</a><span class="date">'+year+'.'+mouth+'.'+day+'</span></li>';
	                    			 });
	                    			 if(rptBbs == ""){
	        							 $("#rpt-bbs-list").append("<li>No Data</li>");
	        						 }else{
						...
						// $("#mna-bbs-cnt").html('共 <span class="num">'+numberWithCommas(data.mnaCnt)+' 件</span>');
						 var mnaBbs = "";
						 $(".ikmp-slider .slider > *").remove();
	                      $.each(data.mnaList,function(i,v){
							 mnaBbs += '<div class="slide">';
							 mnaBbs += '<a href="/ik-ch/pgm/i-633/inquiry/front/project-detail.do?fatsht_sn='+v.fatshtSn+'" target="_balnk" title="새창열림">';
							 mnaBbs += '<div class="cont">';
							// mnaBbs += '<div class="badge">BEST<span>'+(i+1)+'</span></div>';
							 mnaBbs += '<div class="category">'+v.entpIndutyName+'</div>';
							 mnaBbs += '<div class="tit">'+v.pjtCnlngName+'</div>';
							 mnaBbs += '<div class="desc">'+v.techDcCnlngCn+'</div>';
							 mnaBbs += '</div>';
						 ...
						 if(mnaBbs == ""){
							 $("#mna-bbs-list").append("<li>No Recommendation company.</li>");
						 }else{
                     ...
                     var makeSearchGfResult = function(data){
                    	 ...
                    	 if(data.perfrmYn){
                     	 ...
                     	 $.each(data.searchList,function(i,v){
                     		 if(v.Id == 'news'){
                     			 $("#news-cnt").text(v.DocumentSet.TotalCount);
                     			 $("#news-bbs-cnt").html('共<span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+' 件</span>');
                     				 var newsBbs = "";
  	                    			 $.each(v.DocumentSet.Document,function(k,d){
  	                    				var regDt = new Date(d.Date);
 	                    				 var year = regDt.getFullYear();
                     			 ...
                     			 $("#rpt-cnt").text(v.DocumentSet.TotalCount);
                     			 $("#rpt-bbs-cnt").html('共<span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+' 件</span>');
 									var rptBbs = "";
 	                    			 $.each(v.DocumentSet.Document,function(k,d){
 	                    				 var regDt = new Date(d.Date);
 	                    				 var year = regDt.getFullYear();
						...
						// $("#grfi-bbs-cnt").html('共<span class="num">'+numberWithCommas(data.grfiCnt)+' 件</span>');
						 var grfiBbs = "";
                         ...
                         $.each(data.grfiList,function(i,v){
							 ...
							 grfiBbs += '<div class="category">'+v.eng_regn_name+'</div>';
							 grfiBbs += '<div class="tit">'+v.eng_hsmp_name+'</div>';
							 grfiBbs += '<ul class="list02">';
							 grfiBbs += '<li><span class="bold">指定面積(km2) : </span>'+numberWithCommas(v.appn_ar)+'</li>';
							 grfiBbs += '<li><span class="bold">附近的火车站 : </span>'+v.eng_rlroad_name+'</li>';
							 grfiBbs += '<li><span class="bold">附近的机场 : </span>'+v.eng_arprt_val+'</li>';
							 grfiBbs += '</ul>';
							 grfiBbs += '<button type="button" class="btn-position gfri-info" data-seq="'+v.indust_hsmp_cd+'">检视信息</button>';
							grfiBbs += '</div>';
							 grfiBbs += '</a>';
							 grfiBbs += '</div>';
						 });
						 ...
						 if(grfiBbs == ""){
							 $("#grfi-bbs-list").html("<li>No Industrial complex</li>");
						 }else{
                     ...
                     var makeBbsList =function(data){
                    	 $("#popup-total-count").html('共 <span>'+numberWithCommas(data.totCnt)+'</span>件[<span>'+data.paginationInfo.currentPageNo+'</span>/'+data.paginationInfo.totalPageCount+'page]');
                    	 var listHtml = "";
                    	 $.each(data.resultList.Document,function(i,v){
                    		 ...
                    		 listHtml += '<li><span class="bold">作家 : </span>'+v.Field.USER_NAME+'</li>';
                    		 listHtml += '<li><span class="bold">创建日期 : </span>'+year+'.'+month+'.'+day+'</li>';
                    		 listHtml += '<li><span class="bold">点击数 : </span>'+numberWithCommas(v.Field.RDCNT)+'</li>';
                    		 listHtml += '</ul>';
                    		 listHtml += '</div>';
                    		 listHtml += '<a href="'+v.Field.PATH_MENU_URL+'" class="btn-go">link</a>';
                    		 listHtml += '</div>';
                    		 listHtml += '</li>';
                    	 });
                    	 if(listHtml == ""){
                    		 $("#popup-data-list").append("<li>No Data.</li>");
   						 }else{
                     ...
                     var makeDetailResult = function(data){
                    	 $("#grfi-name").text("["+data.eng_regn_name+"] "+data.eng_hsmp_name);
                    	 $("#eng_hsmp_name").text(data.eng_hsmp_name);
                    	 $("#hmpg_url").attr("href",data.hmpg_url);
                    	 $("#appn_de").text(data.appn_de);
                    	 $("#appn_ar").text(numberWithCommas(data.appn_ar));
                    	 $("#mnt_instt_name").text(data.eng_mnt_instt_name);
                    	 $("#eng_rlroad_name").text(data.eng_rlroad_name);
                    	 $("#rlroad_dstnc").text(numberWithCommas(data.rlroad_dstnc));
                    	 $("#eng_arprt_val").text(data.eng_arprt_val);
                    	 $("#arprt_dstnc").text(numberWithCommas(data.arprt_dstnc));
                    	 $("#indust_uswtr_val").text(data.indust_uswtr_val);
                    	 $("#eng_regn_name").text(data.eng_regn_name);
                     ...
                     var step1 = function(data){
                        	...
                        	$.each(data.data,function(i,v){
                        		liHtml += '<li><a href="#" class="indust-code" data-seq="'+v.indust_cl_cd+'" data-step="2">'+v.eng_indust_cl_cd_name+'</a></li>';
                        	});
                        	$("#step2").append(liHtml);
                        ...
                        var step2 = function(data){
                        	...
                        	$("#step4").empty();
                        	$.each(data.data,function(i,v){
                        		liHtml += '<li><a href="#" class="indust-code" data-seq="'+v.indust_cl_cd+'" data-step="3">'+v.eng_indust_cl_cd_name+'</a></li>';
                        	});
                        	$("#step3").append(liHtml);
                        }
                        ...
                        var step3 = function(data){
                        	...
                        	$("#step4").empty();
                        	$.each(data.data,function(i,v){
                        		liHtml += '<li><a href="#" class="last-code" data-seq="'+v.indust_cl_cd+'">'+v.eng_indust_cl_cd_name+'</a></li>';
                        	});
                        	$("#step4").append(liHtml);
                        }
