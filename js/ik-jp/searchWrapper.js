 ...
 // IKMP 매물추천 슬라이더
                  ...
                  function searchBodyGrfiOpen(){
                     ...
                     var makeSearchResult = function(data){
                    	 ...
                    	 $.each(data.searchList,function(i,v){
                    		 if(v.Id == 'news'){
                    			 ...
                    			 $("#news-bbs-cnt").html('計 <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
                    				 var newsBbs = "";
 	                    			 $.each(v.DocumentSet.Document,function(k,d){
	                    				 ...
	                    				 newsBbs += '<li><a href="'+d.Field.PATH_MENU_URL+'" target="_blank">'+d.Field.SJ+'</a><span class="date">'+year+'.'+mouth+'.'+day+'</span></li>';
                     				 });
 	                    			if(newsBbs == ""){
                    			 ...
                    			 $("#rpt-bbs-cnt").html('計 <span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+'data</span>');
									var rptBbs = "";
	                    			 $.each(v.DocumentSet.Document,function(k,d){
	                    				 var regDt = new Date(d.Date);
	                    				 var year = regDt.getFullYear();
	                    				 var mouth = ('0'+(1 + regDt.getMonth())).slice(-2);
	                    				 var day = ('0'+regDt.getDate()).slice(-2);
	                    				 rptBbs += '<li><a href="'+d.Field.PATH_MENU_URL+'" target="_blank">'+d.Field.SJ+'</a><span class="date">'+year+'.'+mouth+'.'+day+'</span></li>';
	                    			 });
	                    			 if(rptBbs == ""){
						 ...
						 //$("#mna-bbs-cnt").html('計 <span class="num">'+numberWithCommas(data.mnaCnt)+' 件</span>');
						 var mnaBbs = "";
						 $(".position-slider .slider > *").remove();
                         $.each(data.mnaList,function(i,v){
							 mnaBbs += '<div class="slide">';
							 mnaBbs += '<a href="/ik-jp/pgm/i-808/inquiry/front/project-detail.do?fatsht_sn='+v.fatshtSn+'" target="_balnk" title="새창열림">';
							 mnaBbs += '<div class="cont">';
							// mnaBbs += '<div class="badge">BEST<span>'+(i+1)+'</span></div>';
							 mnaBbs += '<div class="category">'+v.entpIndutyName+'</div>';
							 mnaBbs += '<div class="tit">'+v.pjtJplngName+'</div>';
							 mnaBbs += '<div class="desc">'+v.techDcJplngCn+'</div>';
							 mnaBbs += '</div>';
                     ...
                     var makeSearchGfResult = function(data){
                    	 ...
                    	 if(data.perfrmYn){
                     	 ...
                     	 $.each(data.searchList,function(i,v){
                     		 if(v.Id == 'news'){
                     			 $("#news-cnt").text(v.DocumentSet.TotalCount);
                     			 $("#news-bbs-cnt").html('計<span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+' 件</span>');
                     				 var newsBbs = "";
  	                    			 $.each(v.DocumentSet.Document,function(k,d){
  	                    				var regDt = new Date(d.Date);
 	                    				 var year = regDt.getFullYear();
                     			 ...
                     			 $("#rpt-cnt").text(v.DocumentSet.TotalCount);
                     			 $("#rpt-bbs-cnt").html('計<span class="num">'+numberWithCommas(v.DocumentSet.TotalCount)+' 件</span>');
 									var rptBbs = "";
 	                    			 $.each(v.DocumentSet.Document,function(k,d){
 	                    				 var regDt = new Date(d.Date);
 	                    				 var year = regDt.getFullYear();
						 ...
						 //$("#grfi-bbs-cnt").html('計<span class="num">'+numberWithCommas(data.grfiCnt)+' 件</span>');
						 var grfiBbs = "";
	                     ...
	                     $.each(data.grfiList,function(i,v){
							...
							// grfiBbs += '<div class="badge">BEST<span>'+(i+1)+'</span></div>';
							 ...
							 grfiBbs += '<li><span class="bold">近くの駅 : </span>'+v.eng_rlroad_name+'</li>';
							 grfiBbs += '<li><span class="bold">近くの空港 : </span>'+v.eng_arprt_val+'</li>';
							 grfiBbs += '</ul>';
							 grfiBbs += '<button type="button" class="btn-position gfri-info" data-seq="'+v.indust_hsmp_cd+'">おすすめの立地情報 もっと見る</button>';
							grfiBbs += '</div>';
							 grfiBbs += '</a>';
							 grfiBbs += '</div>';
						 });
                     ...
                     var makeBbsList =function(data){
                    	 $("#popup-total-count").html('計 <span>'+numberWithCommas(data.totCnt)+'</span>件 [<span>'+data.paginationInfo.currentPageNo+'</span>/'+data.paginationInfo.totalPageCount+'ページ]');
                    	 var listHtml = "";
